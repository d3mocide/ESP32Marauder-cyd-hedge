...
class TDongleHardwareTests(unittest.TestCase):
    ...
    def test_t_dongle_led_uses_guarded_reference_library(self):
        ...
        # This fork's build_parallel.yml builds CYD targets only, so it carries
        # no T-Dongle C5 board entry or install step, and nightly_build.yml is
        # dropped entirely. build_installer_manifests.yml derives its matrix
        # from the full upstream target registry and keeps the step.
        for workflow_name in ("build_installer_manifests.yml",):
